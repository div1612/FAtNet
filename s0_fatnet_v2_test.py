...
print("Loading Data ... ")
data = DataLoader() 
test_x1, test_x2, test_y = data.voxceleb1_test_S0()#data.language_test_S0()#data.aishell_multilingual_test_S0()#data.librispeech_multilingual_test_S0()#data.multilingual_test_S0()#data.multilingual_test_S0()#data.voxceleb1_test_S0()#data.voxceleb1_test_S0()#data.voxceleb1_test_S#for Low threshold 
...
for i in tqdm((range(0,len(test_y), batch_size))): 
    ...
    with torch.no_grad( ) :
        ...
        fatprob = cos_sim(audio1.squeeze().detach().cpu().numpy(), audio2.squeeze().detach().cpu().numpy())
